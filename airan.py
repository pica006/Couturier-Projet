...
def afficher_page_comptabilite_airan():
    ...
    commandes_relance = controller.obtenir_commandes_a_relancer(couturier_id)
    if tri_relance == "Date recente":
        commandes_relance = sorted(commandes_relance, key=lambda x: x["date_creation"], reverse=True)
    elif tri_relance == "Date ancienne":
        commandes_relance = sorted(commandes_relance, key=lambda x: x["date_creation"])
    elif tri_relance == "Reste decroissant":
        commandes_relance = sorted(commandes_relance, key=lambda x: float(x["reste"]), reverse=True)
    elif tri_relance == "Reste croissant":
        commandes_relance = sorted(commandes_relance, key=lambda x: float(x["reste"]))
    elif tri_relance == "Nom client A-Z":
        commandes_relance = sorted(commandes_relance, key=lambda x: f"{x['client_nom']} {x['client_prenom']}".lower())
    else:
        commandes_relance = sorted(
            commandes_relance,
            key=lambda x: f"{x['client_nom']} {x['client_prenom']}".lower(),
            reverse=True,
        )
                ...
                if st.button("Envoyer un rappel par email", key=f"airan_relance_email_{cmd['id']}", width="stretch"):
                    ...
                    if not client_email:
                        ...
                        body = (
                            ...
                            f"Commande: #{cmd['id']}\n"
                            f"Modele: {cmd.get('modele', 'N/A')}\n"
                            f"Prix total: {cmd.get('prix_total', 0):,.0f} FCFA\n"
                            f"Avance: {cmd.get('avance', 0):,.0f} FCFA\n"
                            f"Reste a payer: {cmd.get('reste', 0):,.0f} FCFA\n\n"
                            "Merci pour votre confiance."
                        )
                        attachments = [cmd["pdf_path"]] if cmd.get("pdf_path") else None
                        with st.spinner("Envoi du rappel en cours..."):
                            success, message = email_controller.envoyer_email_avec_message(
                                client_email,
                                subject,
                                body,
